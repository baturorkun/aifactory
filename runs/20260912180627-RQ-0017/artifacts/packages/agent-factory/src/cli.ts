...
// ============================================================
// factory probe <build|board-run|simics-run|compare|phase> <req-id>
// ============================================================

const probe = program
  .command('probe')
  .description('Hardware-twin workflow: build the probe, run it on the board and under Simics, compare');
...
probe
  .command('simics-run <reqId>')
  .description('Run the committed ELF under Simics and capture its trace')
  .action((reqId: string) => {
    try {
      const config = loadConfig();
      const twin = loadTwinRequirement(reqId, config);
      const result = runProbeOnSimics(twin, config);
      console.log(chalk.green(`\n✓ Simics trace captured: ${chalk.bold(twin.probe.name)}`));
      console.log(chalk.dim(`  File    : ${twin.probe.simicsTracePath}`));
      console.log(chalk.dim(`  Lines   : ${result.trace.lines.length} register(s), source ${result.trace.source}`));
      if (twin.phase === 'model') {
...
probe
  ...
  .description('Diff the board trace against the Simics trace')
  .action((reqId: string) => {
    try {
      ...
      if (diff.equal) {
        ...
        console.log(chalk.yellow(`\n⚠ ${diff.differences.length} difference(s) between board and Simics:\n`));
        console.log(text);
        console.log();
        process.exitCode = 1;
      }

  ...
  return [
    ...
    ...(twin
      ? [
          '## Probe',
          '',
          `<!-- What probes/${input.probe}/ reads and prints, one register per bullet: GROUP.REGISTER @0xADDRESS. -->`,
          '',
          '## Behaviour',
          '',
          '<!-- Which actions the probe performs (WRITE, WAIT, MEM) and which observations must follow, in order. A probe that only reads produces a reset-state model, and the profile will say so. -->',
          '',
          '## Board',
          '',
          '<!-- How the board is programmed and captured for this requirement, or "manual" when BOARD_* is not configured. -->',
          '',
          '## Model',
          '',
          '<!-- The devices, targets and profiles to write once the board trace is committed. -->',
          '',
        ]
      : []),
    '## Acceptance Criteria',
...
export async function createDraftRequirement(
  ...
  if (probe !== undefined && !/^[a-z0-9][a-z0-9-]*$/.test(probe)) {
    throw new Error(`Probe name must be a lowercase slug: ${probe}`);
  }
  if (kind !== 'hardware-twin' && options.probe) {
    throw new Error('--probe only applies to --kind hardware-twin.');
  }

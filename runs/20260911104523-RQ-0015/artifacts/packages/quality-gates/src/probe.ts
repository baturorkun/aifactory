...
export function locateProbe(targetRoot: string, name: string): ProbeLocation {
  ...
  return {
    ...
    buildDir,
    simicsTracePath: join(buildDir, SIMICS_TRACE_FILE),
  };
}
...
export function boardParityGate(location: ProbeLocation): GateReport {
  ...
  const simics = readTrace(location.simicsTracePath, `Simics trace ${relative(location.targetRoot, location.simicsTracePath)}`);
  if (!simics.trace) {
    return report('boardParity', start, false, `${simics.error} Run "factory probe simics-run" first.`);
  }
  const diff = compareProbeTraces(board.trace, simics.trace);

...
import { buildProbe, compareProbeRuns, loadTwinRequirement, runProbeOnBoard, runProbeOnSimulator, setTwinPhase } from './probe';
...
test('compareProbeTraces reports value differences and honours volatile', () => {
  ...
  // By default the second side is the twin, whichever simulator ran it.
  assert.match(formatProbeTraceDiff(diff), /board: SYSREG.DEVICE_VERSION[\s\S]*twin : SYSREG.DEVICE_VERSION/);
  assert.doesNotMatch(formatProbeTraceDiff(diff), /simics|renode/);
...
test('probeBuild and boardTrace gates tie the ELF and the trace to the committed sources', () => {
  ...
  try {
    ...
    assert.match(boardParityGate(location).output, /sim-run/);
    mkdirSync(location.buildDir, { recursive: true });
    writeFileSync(location.simulatorTracePath, traceFor(hash, '0x00000001'));
    const parity = boardParityGate(location);
    assert.equal(parity.status, 'failed');
    assert.match(parity.output, /#1 value/);
    writeFileSync(location.simulatorTracePath, traceFor(hash));
...
function twinProject(): { root: string; config: ReturnType<typeof FactoryConfigSchema.parse>; hash: string } {
  ...
  const config = FactoryConfigSchema.parse({
    ...
    targetProject: {
      root,
      commands: {
        // The build "compiles" by writing the marker; probeSimicsRun (the legacy key) exercises the fallback to probeSimulatorRun.
...
test('sim-run captures a trace and compare reports parity', () => {
  const { root, config, hash } = twinProject();
  try {
    const twin = loadTwinRequirement('RQ-0009', config);
    buildProbe(twin, config);
    writeFileSync(twin.probe.boardTracePath, traceFor(hash));
    const run = runProbeOnSimulator(twin, config);
    ...
    assert.match(text, /twin : SYSREG.ESRAM_CR @0x40038000 = 0x00000000/, 'the diff calls the model side the twin');
    assert.doesNotMatch(text, /renode/);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test('a missing machine value is an error naming it, never a default', () => {
  ...
  try {
    ...
    assert.throws(() => runProbeOnSimulator(twin, noBuild), /probeSimulatorRun is not configured/);

...
export function probeEnvironment(twin: TwinRequirement, sourceHash: string, env: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  return {
    ...
    PROBE_TRACE_OUT: twin.probe.simicsTracePath,
  };
}
...
// ------------------------------------------------------------
// simics run and compare
// ------------------------------------------------------------

export interface SimicsRunResult {
  trace: ProbeTrace;
  simicsTraceSha256: string;
}

export function runProbeOnSimics(twin: TwinRequirement, config: FactoryConfig, env: NodeJS.ProcessEnv = process.env): SimicsRunResult {
  const command = config.targetProject.commands.probeSimicsRun;
  if (!command) throw new Error('targetProject.commands.probeSimicsRun is not configured in factory.config.json.');
  if (!existsSync(twin.probe.elfPath)) {
    throw new Error(`${relative(twin.targetRoot, twin.probe.elfPath)} does not exist. Run "factory probe build" first.`);
  }
  const sources = computeProbeSourceHash(twin.probe);
  mkdirSync(twin.probe.buildDir, { recursive: true });
  runShell(command, twin.targetRoot, probeEnvironment(twin, sources.short, env),
    config.targetProject.commandTimeoutMs ?? 120_000, 'probe simics-run');
  const rel = relative(twin.targetRoot, twin.probe.simicsTracePath);
  if (!existsSync(twin.probe.simicsTracePath)) {
    throw new Error(`The command finished but ${rel} does not exist. It must write the captured trace to $PROBE_TRACE_OUT.`);
  }
  const text = readFileSync(twin.probe.simicsTracePath, 'utf8');
  const trace = parseProbeTrace(text);
  ...
  writeFileSync(twin.probe.simicsTracePath, normalizeTraceText(text), 'utf8');
  return { trace, simicsTraceSha256: fileSha256(twin.probe.simicsTracePath) };
}

export function compareProbeRuns(twin: TwinRequirement): { diff: ProbeTraceDiff; text: string } {
  const boardRel = relative(twin.targetRoot, twin.probe.boardTracePath);
  const simicsRel = relative(twin.targetRoot, twin.probe.simicsTracePath);
  if (!existsSync(twin.probe.boardTracePath)) throw new Error(`${boardRel} is missing. Run "factory probe board-run" first.`);
  if (!existsSync(twin.probe.simicsTracePath)) throw new Error(`${simicsRel} is missing. Run "factory probe simics-run" first.`);
  const board = parseProbeTrace(readFileSync(twin.probe.boardTracePath, 'utf8'));
  const simics = parseProbeTrace(readFileSync(twin.probe.simicsTracePath, 'utf8'));
  const diff = compareProbeTraces(board, simics);

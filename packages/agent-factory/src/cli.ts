...
import { createTargetProject, PROJECT_TEMPLATES, SIMULATORS } from './scaffold';
...
import type { RequirementExecutionMode, RequirementKind, TwinPhase } from '@aifactory/contracts';
import {
  buildProbe,
  compareProbeRuns,
  loadTwinRequirement,
  ...
  setTwinPhase,
  writeTwinPhase,
} from './orchestrator/probe';
import { TWIN_PHASES } from '@aifactory/contracts';
...
// ============================================================
// factory probe <build|board-run|sim-run|compare|phase> <req-id>
// ============================================================

const probe = program
  .command('probe')
  .description('Hardware-twin workflow: build the probe, run it on the board and under the simulator, compare');
...
probe
  .command('build <reqId>')
  .description('Compile the probe on the licensed host with its source hash embedded, and place the ELF beside its sources')
  .action((reqId: string) => {
    try {
      const config = loadConfig();
      const twin = loadTwinRequirement(reqId, config);
      const result = buildProbe(twin, config);
      console.log(chalk.green(`\n✓ Probe built: ${chalk.bold(twin.probe.name)}`));
      console.log(chalk.dim(`  Sources : ${result.files.length} file(s), hash ${result.sourceHashShort}`));
      console.log(chalk.dim(`  ELF     : ${twin.probe.elfPath}`));
      console.log(chalk.dim(`  SHA-256 : ${result.elfSha256}`));
      if (twin.phase === 'probe') {
        setTwinPhase(twin, 'board');
        console.log(chalk.dim('  Phase   : probe → board. Commit the ELF with the sources, then run: factory probe board-run ' + reqId));
      }
      console.log();
    } catch (err) { fail(err); }
  });
...
probe
  .command('board-run <reqId>')
  .description('Program the real board with the committed ELF and record its trace (manual mode when BOARD_* is unset)')
  .option('--wait <seconds>', 'Manual mode: stop waiting for the trace file after this long')
  .action(async (reqId: string, opts: { wait?: string }) => {
    try {
      const config = loadConfig();
      const twin = loadTwinRequirement(reqId, config);
      const result = await runProbeOnBoard(twin, {
        waitMs: opts.wait === undefined ? undefined : Number(opts.wait) * 1000,
      });
      console.log(chalk.green(`\n✓ Board trace recorded (${result.mode}): ${chalk.bold(twin.probe.name)}`));
      console.log(chalk.dim(`  File    : ${twin.probe.boardTracePath}`));
      console.log(chalk.dim(`  Lines   : ${result.trace.lines.length} register(s), source ${result.trace.source}`));
      console.log(chalk.dim(`  SHA-256 : ${result.boardTraceSha256}`));
      if (TWIN_PHASES.indexOf(twin.phase) < TWIN_PHASES.indexOf('model')) {
        setTwinPhase(twin, 'model');
        console.log(chalk.dim('  Phase   : → model. Commit the trace, then implement the model and finish the handoff.'));
      }
      console.log();
    } catch (err) { fail(err); }
  });
...
probe
  // sim-run is the neutral name; simics-run stays as a hidden alias for
  // projects and muscle memory from before RQ-0019.
  .command('sim-run <reqId>', { hidden: false })
  .aliases(['simics-run'])
  .description('Run the committed ELF under the simulator and capture its trace')
  .action((reqId: string) => {
    try {
      const config = loadConfig();
      const twin = loadTwinRequirement(reqId, config);
      ...
      console.log(chalk.dim(`  Lines   : ${result.trace.lines.length} register(s), source ${result.trace.source}`));
      if (twin.phase === 'model') {
        setTwinPhase(twin, 'parity');
        console.log(chalk.dim('  Phase   : model → parity. Run: factory probe compare ' + reqId));
      }
      console.log();
    } catch (err) { fail(err); }
  });
...
probe
  .command('compare <reqId>')
  .description('Diff the board trace against the simulator trace')
  .action((reqId: string) => {
    try {
      const config = loadConfig();
      const twin = loadTwinRequirement(reqId, config);
...
probe
  .command('phase <reqId> [phase]')
  .description('Show the hardware-twin phase, or set it deliberately (e.g. back to probe after changing the probe)')
  .action((reqId: string, phase?: string) => {
    try {
      const twin = loadTwinRequirement(reqId, loadConfig());
      if (phase === undefined) {
        console.log(`${reqId}: phase ${chalk.bold(twin.phase)}, probe ${twin.probe.name}`);
        return;
      }
      if (!TWIN_PHASES.includes(phase as TwinPhase)) {
        throw new Error(`Unknown phase "${phase}". Choose one of: ${TWIN_PHASES.join(', ')}.`);
      }
      writeTwinPhase(twin, phase as TwinPhase);
      console.log(chalk.green(`✓ ${reqId}: phase set to ${chalk.bold(phase)}`));
    } catch (err) { fail(err); }
  });
...
program
  ...
  .action((runId: string, opts: { by: string }) => {
    try {
      ...
      // A hardware-twin run is only done when the simulator matched the board.
      if (manifest.twin && manifest.gateResults.boardParity !== 'passed') {
        console.error(chalk.yellow(
          `Hardware-twin run ${runId} was gated in phase "${manifest.twin.phase}" and its boardParity gate is ` +
          `${manifest.gateResults.boardParity ?? 'not run'}. Reach the parity phase and finish the handoff again before approving.`,
        ));
        process.exit(1);
      }
...
program
  ...
  .option('--template <name>', 'Standard project skeleton: ' + PROJECT_TEMPLATES.join(', '))
  .option('--simulator <name>', 'Hardware-twin project for a simulator: ' + SIMULATORS.join(', '))
  .option('--host-type <os>', 'OS the simulator and build run on: windows, linux')
  .option('--dir <path>', 'Parent directory for the new project', '..')
  .option('--force', 'Allow writing into an existing directory', false)
  .action((projectName: string, opts: { template?: string; simulator?: string; hostType?: string; dir: string; force: boolean }) => {
    try {
      if (opts.template && opts.simulator) {
        throw new Error('--template (standard project) and --simulator (hardware-twin project) are mutually exclusive.');
      }
      ...
      console.log(chalk.dim(result.simulator ? "  Simulator: " + result.simulator : "  Template : " + result.template));

...
function generatedProject(name: string): { root: string; parent: string } {
  ...
  writeFileSync(join(projectRoot, '.env'), 'SIMICS_REMOTE_HOST=\n', 'utf8');
...
test('remote mode names the missing variable and never invents a default', () => {
  ...
  try {
    const noUser = transport(root, ['--', 'sh', '-c', 'true'], { SIMICS_REMOTE_HOST: 'host.example' });
    assert.equal(noUser.status, 2);
    assert.match(noUser.output, /SIMICS_REMOTE_USER is not set, but SIMICS_REMOTE_HOST names a remote Simics host/);

    const noBase = transport(root, ['--', 'sh', '-c', 'true'],
      { SIMICS_REMOTE_HOST: 'host.example', SIMICS_REMOTE_USER: 'someone' });
    assert.match(noBase.output, /SIMICS_REMOTE_BASE_PATH is not set/);

    const badBase = transport(root, ['--', 'sh', '-c', 'true'], {
      SIMICS_REMOTE_HOST: 'host.example', SIMICS_REMOTE_USER: 'someone',
      SIMICS_REMOTE_BASE_PATH: '/not/windows', SIMICS_REMOTE_PROJECT_NAME: 'p',
    });
    assert.match(badBase.output, /SIMICS_REMOTE_BASE_PATH must be an absolute Windows/);

    const badPort = transport(root, ['--', 'sh', '-c', 'true'], {
      SIMICS_REMOTE_HOST: 'host.example', SIMICS_REMOTE_USER: 'someone',
      SIMICS_REMOTE_BASE_PATH: 'C:\\w', SIMICS_REMOTE_PROJECT_NAME: 'p', SIMICS_REMOTE_PORT: '0',
    });
    assert.match(badPort.output, /SIMICS_REMOTE_PORT must be an integer/);
  } finally {
    rmSync(parent, { recursive: true, force: true });
  }
});

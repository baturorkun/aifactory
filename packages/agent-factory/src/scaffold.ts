...
function writeGitlabCi(projectRoot: string, projectName: string, deployable: boolean): void {
  writeFileSync(
    ...
    [
      ...
      '            test -r "$CODEX_HOME/auth.json" || { echo "codex-cli provider requires readable $CODEX_HOME/auth.json from the runner mount"; exit 1; }',
      '            codex login status >/dev/null || { echo "Codex CLI authentication is invalid; refresh $CODEX_HOME/auth.json on the runner host"; exit 1; }',
      '          fi',

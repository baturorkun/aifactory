...
function writeSimicsTemplate(projectRoot: string, projectName: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '  const recorded = config.gates?.[mode];',
      '  if (!recorded) {',
      '    console.error(`No "gates.${mode}" entry in ${CONFIG_FILE} and no ${variable}; licensed Simics validation was not run.`);',
      '    process.exit(2);',
      '  }',
      '  source = `${CONFIG_FILE} (gates.${mode})`;',

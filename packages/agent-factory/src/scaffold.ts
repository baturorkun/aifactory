...
export function createTargetProject(projectName: string, options: NewProjectOptions): NewProjectResult {
  ...
  writeFileSync(
    ...
    [
      ...
      '- Change a requirement execution mode with `factory requirement mode <requirement-id> <pipeline|handoff|direct>`; the CLI command is `mode`, not `set-mode`.',
      '- Run a ready direct-mode requirement locally with `factory direct <requirement-id>`; it uses one workspace-writing Codex CLI pass and then the configured quality gates.',

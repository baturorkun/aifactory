  ...
  writeFileSync(
    ...
    [
      ...
      ...(deployable
        ? [
            '  APP_PORT: "8282"',
            '  APP_PREVIEW_PORT: "8283"',
            '  LOCAL_DOCKER_IMAGE: "' + projectName + ':$CI_COMMIT_SHORT_SHA"',
            '  CONTAINER_NAME: "' + projectName + '"',
            '  PREVIEW_CONTAINER_NAME: "' + projectName + '-preview"',
          ]
        : []),
      '',
      'cache:',
      ...
      ...(deployable
        ? [
            ...
            '  script:',
            '    - docker load --input ' + projectName + '-image.tar',
            '    - docker rm --force "$PREVIEW_CONTAINER_NAME" || true',
            '    - |',
            '      PORT_OWNER="$(docker ps --filter "publish=$APP_PREVIEW_PORT" --format \'{{.Names}}\' | head -n 1)"',
            '      if [ -n "$PORT_OWNER" ]; then',
            '        echo "Preview port $APP_PREVIEW_PORT is already used by container: $PORT_OWNER"',
            '        echo "Set APP_PREVIEW_PORT to a free host port and retry deploy_preview_linux."',
            '        exit 1',
            '      fi',
            '    - docker run --detach --restart unless-stopped --name "$PREVIEW_CONTAINER_NAME" --publish "$APP_PREVIEW_PORT:8282" "$LOCAL_DOCKER_IMAGE"',
            '    - |',
            '      ATTEMPT=0',

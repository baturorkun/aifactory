...
function writeProbeTemplate(projectRoot: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      'WAIT  <GROUP>.<REG> @0x<addr> mask=0x<m> expect=0x<e> -> ok|timeout spins=<n>[ volatile]',
  ...
  writeFileSync(
    ...
    [
      ...
      'typedef enum { PROBE_READ, PROBE_WRITE, PROBE_WAIT, PROBE_MEM } probe_kind_t;',
      '',
      'typedef struct {',
      '    probe_kind_t kind;',
      '    const char *group;',
  ...
  writeFileSync(
    ...
    [
      ...
      '}',
      '',
      'static void run_wait(const probe_step_t *step)',
      '{',
      '    uint32_t spins = 0;',
      '    int ok = 0;',
      '    while (spins < step->limit) {',
      '        if ((REG32(step->address) & step->mask) == step->value) { ok = 1; break; }',
      '        spins++;',
      '    }',
      '    put_string("WAIT "); put_register(step);',
      '    put_string(" mask=0x"); put_hex32(step->mask); put_string(" expect=0x"); put_hex32(step->value);',
      ...
      '        case PROBE_WAIT:  run_wait(step); break;',
